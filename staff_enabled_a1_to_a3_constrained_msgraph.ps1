...
$logFile = "D:\CRON\Office365\LOG\A1_TO_A3_CONSTRAINED\LicenseChangeLog_$logDate.txt"
...
$group = Get-ADGroup -Identity "Google_Enterprise_Licensing_Staff" -Properties Members
$onPremGroupMembers = $group.Members | Get-ADUser

# Get users from Azure AD with specified properties
$azureAdUsers = Get-MgBetaUser -Filter "accountEnabled eq true AND jobtitle eq 'Staff'" -All | Where-Object { $_.AssignedLicenses.SkuId -eq $a1PlusLicenseSkuId }


$enabledUsersWithA1 = foreach ($user in $azureAdUsers) {
    if ($onPremGroupMembers.UserPrincipalName -contains $user.UserPrincipalName) {
        $user
    }
}
